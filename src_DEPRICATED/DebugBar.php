...
namespace Wizdam\DebugBar;

use Wizdam\DebugBar\Collectors\Config;
use Wizdam\DebugBar\Collectors\History;

/**
 * Wizdam Frontedge DebugBar
 *
...
class DebugBar
{
    ...
    /** @var list<\Wizdam\DebugBar\Collectors\BaseCollector> */
    protected array $collectors = [];

    public function __construct(array $config = [])
    {
        $defaults     = require __DIR__ . '/../config/debugbar.php';
        $this->config = array_merge($defaults, $config);

        foreach ($this->config['collectors'] as $collectorClass) {
            if (! class_exists($collectorClass)) {
                error_log('WizdamDebugBar: Collector tidak ditemukan (' . $collectorClass . ').');
                continue;
            }
